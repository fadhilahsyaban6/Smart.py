...
# ==================== HALAMAN KALKULATOR ====================
    ...
    if calc_type == "📊 Kerapatan (Density)":
        ...
        if st.button("🔍 Hitung Kerapatan", type="primary"):
            if volume == 0:
                st.error("❌ Hasil Error! Volume (V) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
    ...
    # --- KALKULATOR VISKOSITAS ---
        ...
        if st.button("🔍 Hitung Viskositas", type="primary"):
            if t_ref == 0 or rho_ref == 0:
                st.error("❌ Hasil Error! Waktu alir referensi (t₁) dan densitas referensi (ρ₁) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
...
- Air suling 25 °C: η = 0.00089 Pa*s, ρ = 997 kg/m³
    ...
    # --- KALKULATOR SUDUT REPOSISI ---
        ...
        if st.button("🔍 Hitung Sudut Reposisi", type="primary"):
            if r == 0:
                st.error("❌ Hasil Error! Jari-jari dasar tumpukan (r) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
    ...
    # --- KALKULATOR PERSAMAAN GAS IDEAL ---
        ...
        if cari == "🔵 Tekanan (P)":
            ...
            if st.button("🔍 Hitung Volume", type="primary"):
                if P_val == 0:
                    st.error("❌ Hasil Error! Tekanan (P) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
            ...
            if st.button("🔍 Hitung Jumlah Mol", type="primary"):
                if T_val == 0:
                    st.error("❌ Hasil Error! Suhu (T) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
            ...
            if st.button("🔍 Hitung Suhu", type="primary"):
                if n_val == 0:
                    st.error("❌ Hasil Error! Jumlah mol (n) berada pada posisi penyebut, sehingga tidak boleh bernilai 0.")
...
- Fahrenheit -> Celcius: C = (F - 32) * 5/9
                    ...
                    st.write(f"{val} {from_u} * {UNIT_TABLE[cat][from_u]} = {sci_fmt(base_val)} (unit dasar SI)")
                    st.markdown("**Langkah 2: Konversi ke Satuan Target**")
                    st.write(f"Faktor konversi unit dasar ke {to_u} = 1/{UNIT_TABLE[cat][to_u]}")
                    st.write(f"{sci_fmt(base_val)} / {UNIT_TABLE[cat][to_u]} = **{result:.6g} {to_u}**")
        else:
