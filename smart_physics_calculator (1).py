...
# ==================== HALAMAN RUMUS & CHEAT SHEET ====================
    ...
    with tab2:
        ...
        for name, data in PHYSICS_CONSTANTS.items():
            st.info(f"**{name}** = {data['nilai']:.4e} {data['satuan']}")
    ...
    with tab3:
        ...
        rumus_categories = {
            ...
            "Listrik & Magnet": [
                ...
                ("Hukum Coulomb", "F = k q_1 q_2/r^2", "Gaya elektrostatik = konstanta * muatan^2/jarak^2"),
        ...
        if st.button("🔍 Hitung Kerapatan", type="primary"):
            if mass <= 0 or volume <= 0:
                ...
                with st.container(border=True):
                    ...
                    st.write(f"Volume = {volume} {vol_unit} = {vol_m3:.6e} m^3")

                with st.container(border=True):
                    st.markdown(f"**Langkah 2:** Masukkan ke rumus kerapatan")
                    st.write(f"ρ = m / V = {mass_kg:.6f} / {vol_m3:.6e}")

                with st.container(border=True):
                    st.markdown(f"**Langkah 3:** Hasil akhir")
                    st.metric("Kerapatan (ρ)", f"{density:.4f} kg/m^3")
                    st.write(f"= {density / 1000:.4f} g/cm^3")
...
**Interpretasi Fisika:**
- Air murni: ρ ~= 1000 kg/m^3 (pada 4 °C)
- Udara: ρ ~= 1.225 kg/m^3 (pada 15 °C, 1 atm)
- Besi: ρ ~= 7870 kg/m^3
- Kerapatan relatif (specific gravity) = ρ_zat / ρ_air
    ...
    # --- KALKULATOR VISKOSITAS ---
        ...
        st.code("η_uji = (t_uji * ρ_uji * η_ref) / (t_ref * ρ_ref)", language=None)
        ...
        with col1:
            ...
            rho_uji = st.number_input("Densitas fluida uji (ρ₂) dalam kg/m^3:", min_value=0.0, value=1020.0, step=1.0, format="%.2f")
        ...
        with col2:
            ...
            rho_ref = st.number_input("Densitas fluida referensi (ρ₁) dalam kg/m^3:", min_value=0.0, value=1000.0, step=1.0, format="%.2f")
        ...
        if st.button("🔍 Hitung Viskositas", type="primary"):
            if t_ref <= 0 or rho_ref <= 0:
                ...
                with st.container(border=True):
                    ...
                    st.write(f"η₂ = {t_uji * rho_uji * eta_ref:.6e} / {t_ref * rho_ref:.2f} = **{eta_uji:.6f} Pa*s**")
...
**Referensi:**
- Air suling 20 °C: η = 0.001002 Pa*s, ρ = 1000 kg/m^3
- Air suling 25 °C: η = 0.00089 Pa*s, ρ = 997 kg/m^3
                """)
                ...
                ref_data = {
                    ...
                    "ρ (kg/m^3)": [1000, 997, 1020, 1420, 890, 1110, 1260, 750, 789]
                }
                st.table(ref_data)
    ...
    # --- KALKULATOR SUDUT REPOSISI ---
        ...
        st.code("tan theta = h / r\ntheta = arctan (h / r)", language=None)
        st.caption("theta = sudut reposisi | h = tinggi sampel tumpukan | r = jari-jari dasar tumpukan")
            ...
            if st.button("🔍 Hitung Tekanan", type="primary"):
                if n_val <= 0 or V_val <= 0:
                    ...
                    with st.expander("📋 Lihat Langkah Pengerjaan"):
                        ...
                        st.write(f"V = {V_val} {V_unit} -> {V_m3:.6e} m^3")
                        st.markdown("Rumus: **P = nRT / V**")
                        st.write(f"P = ({n_val} * {R_SI} * {T_k:.2f}) / {V_m3:.6e} = **{P_pa:.4f} Pa**")
...
# ==================== HALAMAN UNIT CONVERTER ====================
    ...
    if st.button("🚀 Konversi Sekarang", type="primary", use_container_width=True):
        ...
        if result is not None:
            ...
            with col_left:
                st.metric("Dari", f"{val} {from_u}")
            with col_mid:
                st.markdown("###")
                st.write("⬇️")
            with col_right:
                st.metric("Ke", f"{result:.6g} {to_u}")
...
- Fahrenheit -> Celcius: C = (F - 32) * 5/9
                    """)
                    st.write(f"**Hasil:** {val} °{from_u} = {result:.4f} °{to_u}")
                else:
                    ...
                    st.write(f"{val} {from_u} * {UNIT_TABLE[cat][from_u]} = {base_val:.6e} (unit dasar SI)")
                    st.markdown("**Langkah 2: Konversi ke Satuan Target**")
                    st.write(f"Faktor konversi unit dasar ke {to_u} = 1/{UNIT_TABLE[cat][to_u]}")
                    st.write(f"{base_val:.6e} / {UNIT_TABLE[cat][to_u]} = **{result:.6g} {to_u}**")
        else:
...
# ==================== HALAMAN QUIZ ====================
    ...
    QUIZ_QUESTIONS = [
        ...
        },
        {
            ...
            "jawaban": "1000 kg/m^3",
            "pembahasan": "Air pada suhu 4 derajat C memiliki kerapatan maksimum sebesar 1000 kg/m^3 atau 1 g/cm^3."
        },
        {
            "type": "isian",
        ...
        {
            "type": "pilihan_ganda",
            "soal": "Tekanan hidrostatik di kedalaman 10 m dalam air (ρ = 1000 kg/m^3) adalah...",
        ...
        },
        {
            ...
            "jawaban": "0.0013",
            "pembahasan": "F = G(m_1*m_2)/r^2 = 6.67*10^-11 * (1000*2000)/100 = 1.334*10^-6 N. Jika dibulatkan: 0.0013 N"
        },
        {
            "type": "pilihan_ganda",
