    ...
    with tab3:
        ...
        rumus_categories = {
            ...
            "Fluida": [
                ...
                ("Hukum Bernoulli", "P + 1/2 rho v^2 + rho g h = konstan", "Energi per satuan volume konstan sepanjang aliran"),
...
# ==================== HALAMAN KALKULATOR ====================
    ...
    if calc_type == "📊 Kerapatan (Density)":
        st.subheader("📊 Kalkulator Kerapatan (rho = m/V)")
        ...
        if st.button("🔍 Hitung Kerapatan", type="primary"):
            if mass <= 0 or volume <= 0:
                ...
                with st.container(border=True):
                    ...
                    st.write(f"rho = m / V = {mass_kg:.6f} / {vol_m3:.6e}")

                with st.container(border=True):
                    st.markdown(f"**Langkah 3:** Hasil akhir")
                    st.metric("Kerapatan (rho)", f"{density:.4f} kg/m^3")
...
- Kerapatan relatif (specific gravity) = rho_zat / rho_air
    ...
    # --- KALKULATOR VISKOSITAS ---
        ...
        st.code("eta_uji = (t_uji * rho_uji * eta_ref) / (t_ref * rho_ref)", language=None)
        st.caption("eta = viskositas | t = waktu alir | rho = densitas/kerapatan")
        ...
        if st.button("🔍 Hitung Viskositas", type="primary"):
            if t_ref <= 0 or rho_ref <= 0:
                ...
                with st.container(border=True):
                    ...
                    st.write(f"rho_2 (densitas uji) = {rho_uji:.2f} kg/m^3")
                    st.write(f"t_1 (waktu alir referensi) = {t_ref:.2f} detik")
                    st.write(f"rho_1 (densitas referensi) = {rho_ref:.2f} kg/m^3")
                    st.write(f"eta_1 (viskositas referensi) = {eta_ref:.6f} Pa*s")

                with st.container(border=True):
                    st.markdown("**Langkah 2:** Masukkan ke rumus Oswald")
                    st.code(f"eta_2 = (t_2 * rho_2 * eta_1) / (t_1 * rho_1)", language=None)
                    st.write(f"eta_2 = ({t_uji:.2f} * {rho_uji:.2f} * {eta_ref:.6f}) / ({t_ref:.2f} * {rho_ref:.2f})")
                ...
                with st.container(border=True):
                    ...
                    st.write(f"eta_2 = {t_uji * rho_uji * eta_ref:.6e} / {t_ref * rho_ref:.2f} = **{eta_uji:.6f} Pa*s**")
